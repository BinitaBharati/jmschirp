$(document).ready(function () 
{
                 ...
                 var callBackInputObj = {jmsConnectionName : $("#connectionName").val()};
                  ...
                  if(saveBtnAction == 0)//CreateConnection
                  {
                      ...
                      else
                      {
                                 ...
                                 makeAjaxPOSTRequest('groovy/scripts/saveConnection.groovy',postData,processSaveConnectionCallBack, callBackInputObj );
                                 $( this ).dialog( "close" );
                      }
                  } 
                  else//EditConnection
     ...
     function updateJmsConnectionInfo(jmsConnectionInfo)
   {
              ...
              if(jsonFormattedResponse)
              {
                  ...
                  else
                  {
                      for (i = 0 ; i < jsonFormattedResponse.length ; i++)
                      {
                        ...
                        updateJmsConnectionInfo({jmsConnectionId: 'tabConnection-'+i,itemCount:i, jmsConnectionName:jsonFormattedResponse[i].connectionName});
                      }
                  }
    ...
    function populateJmsVendorInfoCallBack(xmlhttp)
    {
        if (xmlhttp.status == 200)
        {
            ...
            for (i = 0 ; i < jsonFormattedResponse.length ; i++)
            {
                var jmsVendorType = jsonFormattedResponse[i].vendorType;

                var jmsVendorVersion = jsonFormattedResponse[i].vendorVersion;
   ...
   $(document).on("mouseenter", ".connectionsListItem", function(e)
   {
    ...
    $(document).on("mouseleave", ".connectionsListItem", function(e){
     ...
     $("#newConnection").click(function(e){
        ...
        //makeAjaxGETRequest('groovy/scripts/getVendorDetails.groovy',populateJmsVendorInfoCallBack);
    ...
    $('#connectionsUL').on('click', '.editConnection', function(e){
        ...
        makeAjaxGETRequest('groovy/scripts/getConnectionDetails.groovy?name='+connectionName,getConnectionDetailsCallBack);
    });
        ...
        errorDialog.dialog({dialogClass: "errorDialog", 
          ...
          buttons: 
          {
                ...
                $( this ).dialog( "close" );
                        ...
                        makeAjaxGETRequest('groovy/scripts/deleteConnection.groovy?name='+connectionName,deleteConnectionCallBack, deleteConnectionCallBackInputObj );
          }}});
    ...
    function deleteConnectionCallBack(xmlhttp, deleteConnectionCallBackInputObj)
    {
        ...
        {
            var jsonFormattedResponse = JSON.parse(xmlhttp.responseText);
            console.log('deleteConnectionCallBack: jsonFormattedResponse = '+jsonFormattedResponse);
    ...
    function getConnectionDetailsCallBack(xmlhttp)
   {
     ...
     makeAjaxGETRequest('groovy/scripts/listConnections.groovy',listConnectionCallBack);
